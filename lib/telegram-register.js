...
export function buildBasketMessage(store, basket, totals = null, opts = {}) {
  const lines = [];
  ...
  const footer = buildTotalsFooter(totals, opts);
  if (footer) lines.push('', footer);

  return lines.join('\n');
}

// The store's day so far, straight from NRS, so the owner never has to add
// up the feed or open the POS to know where the day stands.
export function buildTotalsFooter(totals, { showCancelled = false } = {}) {
  if (!totals || !Number.isFinite(totals.sales_cents)) return null;
  const out = [`📊 <b>Today: ${money(totals.sales_cents)}</b> · ${totals.baskets} sale${totals.baskets === 1 ? '' : 's'}`];

  const cash = totals.cash_cents || 0;
  const card = totals.card_cents || 0;
  if (cash || card) out.push(`💵 Cash ${money(cash)} · 💳 Card ${money(card)}`);

  // Only for the stores being watched closely — elsewhere it is a line of
  // noise on every message. Shown even at zero once switched on, because "no
  // cancellations today" is itself the thing being checked.
  if (showCancelled && totals.cancelled_count != null) {
    const n = totals.cancelled_count;
    out.push(`❌ <b>Cancelled today: ${money(totals.cancelled_cents)}</b> · ${n} sale${n === 1 ? '' : 's'}`);
    if (totals.voided_count) {
      out.push(`🚫 Voided: ${money(totals.voided_cents)} · ${totals.voided_count} item${totals.voided_count === 1 ? '' : 's'}`);
    }
  }
...
export function buildEventMessage(store, event, totals = null, opts = {}) {
  ...
  // A cancellation alert is the one place a running cancelled total belongs:
  // it turns a single event into today's pattern.
  const footer = buildTotalsFooter(totals, opts);
