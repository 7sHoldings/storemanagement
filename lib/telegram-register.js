...
export function buildBasketMessage(store, basket) {
  ...
  lines.push(`${tag.icon} <b>${escapeHtml(tag.label)}</b> · ${escapeHtml(store.name)}${time ? ` · ${time}` : ''}`);
...
export function buildEventMessage(store, event) {
  ...
  const out = [`${style.icon} <b>${escapeHtml(style.label)}</b> · ${escapeHtml(store.name)}${time ? ` · ${time}` : ''}`];
