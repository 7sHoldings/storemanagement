...
async function nrsGet(path) {
  assertToken();
  const url = `${NRS_BASE}/${NRS_TOKEN}/${path}`;
  const res = await fetch(url, { headers: NRS_HEADERS, cache: 'no-store' });
  if (!res.ok) {
    const text = await res.text().catch(() => '');
    throw new Error(`NRS GET ${path.split('?')[0]} → ${res.status}: ${text.slice(0, 200)}`);
  }
  return res.json();
}
...
async function nrsPost(path, body) {
  ...
  if (!res.ok) {
    throw new Error(`NRS POST ${path.split('?')[0]} → ${res.status}: ${JSON.stringify(json).slice(0, 200)}`);
  }
  return json;
}
