...
export function buildBuyingPacingMessage(pacing) {
  ...
  let msg = `🛒 <b>BUYING vs SALES — ${pacing.monthName}</b>\n`;
  msg += `📅 Through day ${pacing.daysElapsed}/${pacing.daysInMonth}\n`;
  msg += `━━━━━━━━━━━━━━━━━━━━\n`;
  msg += `💰 Sales (MTD): <b>${f(o.mtdSales)}</b>\n`;
  msg += `🛒 Buying (MTD): <b>${f(o.mtdBuying)}</b>\n`;
  msg += `📊 Buying is <b>${pctStr(o.pct)}</b> of sales — target ≤ ${o.targetPct.toFixed(0)}% ${flag(o.overBudget)}\n`;
  if (o.headroom >= 0) msg += `✅ Headroom left this month at target: <b>${f(o.headroom)}</b>\n`;
  else msg += `⚠️ Over target by <b>${f(-o.headroom)}</b> — cut back this weekend\n`;
  msg += `\n📈 <b>Projected month-end</b> (current pace):\n`;
  msg += `   Sales ~${f(o.projSales)} · Buying ~${f(o.projBuying)} (${pctStr(o.projPct)}) ${flag(o.projOverBudget)}\n`;
  if (o.projOverBudget) msg += `   👉 On pace to overspend on buying — buy less this trip.\n`;

  if (pacing.perStore.length > 1) {
    msg += `\n━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `<b>BY STORE</b> (MTD sales → bought, % of sales / target)\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━\n`;
    for (const s of pacing.perStore) {
      const short = (s.name || '').split(' - ').pop()?.trim() || s.name;
      msg += `🏪 <b>${short}</b> — ${f(s.mtdSales)} → ${f(s.mtdBuying)} (${pctStr(s.pct)} / ${s.targetPct.toFixed(0)}%) ${flag(s.overBudget)}\n`;
      if (s.headroom < 0) msg += `   over target by ${f(-s.headroom)}\n`;
    }
  }
