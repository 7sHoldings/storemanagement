...
// Wednesday "buying vs sales" pacing report. `pacing` is the object returned
// by computeBuyingPacing() in lib/buying-pacing.js.
export function buildBuyingPacingMessage(pacing) {
  const f = formatCurrency;
  const pctStr = (v) => (v == null ? '—' : `${v.toFixed(1)}%`);
  const flag = (over) => (over ? '🔴' : '🟢');
  const o = pacing.overall;
  const year = (pacing.asOf || '').slice(0, 4);
  const dateRange = `${pacing.monthName} 1–${pacing.daysElapsed}${year ? `, ${year}` : ''}`;

  let msg = `🛒 <b>BUYING vs SALES</b> — ${pacing.monthName}${year ? ` ${year}` : ''}\n`;
  msg += `📅 ${dateRange} · ${pacing.daysElapsed} of ${pacing.daysInMonth} days\n`;
  msg += `━━━━━━━━━━━━━━━━━━━━\n`;
  msg += `💰 Sales (MTD): <b>${f(o.mtdSales)}</b>\n`;
  msg += `🛒 Buying (MTD): <b>${f(o.mtdBuying)}</b>\n`;
  msg += `🎯 Target: buying ≤ ${o.targetPct.toFixed(0)}% of sales = <b>${f(o.targetBuying)}</b>\n`;
  if (o.pct == null) {
    msg += `📊 No sales recorded yet this month.\n`;
  } else if (o.overBudget) {
    msg += `📊 Actual: <b>${pctStr(o.pct)}</b> 🔴 — over budget by <b>${f(-o.headroom)}</b>\n`;
    msg += `   👉 Hold off / cut back on buying this weekend.\n`;
  } else {
    msg += `📊 Actual: <b>${pctStr(o.pct)}</b> 🟢 — <b>${f(o.headroom)}</b> still to spend this month at target\n`;
  }

  msg += `\n📈 <b>Projected for all of ${pacing.monthName}${year ? ` ${year}` : ''}</b> (current pace):\n`;
  msg += `   Sales ~${f(o.projSales)} · Buying ~${f(o.projBuying)} (${pctStr(o.projPct)}) ${flag(o.projOverBudget)}\n`;
  if (o.projOverBudget) msg += `   Target ~${f(o.projTargetBuying)} → on pace to overspend by ~${f(o.projBuying - o.projTargetBuying)}.\n`;
  else if (o.projPct != null) msg += `   Target ~${f(o.projTargetBuying)} → on track.\n`;

  if (pacing.perStore.length > 1) {
    const ordered = [...pacing.perStore].sort((a, b) => (b.pct ?? -1) - (a.pct ?? -1));
    msg += `\n━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `<b>BY STORE</b> — MTD sales → bought (worst first)\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━\n`;
    for (const s of ordered) {
      const short = (s.name || '').split(' - ').pop()?.trim() || s.name;
      const tail = s.headroom < 0 ? `over by ${f(-s.headroom)}` : `${f(s.headroom)} to spare`;
      msg += `🏪 <b>${short}</b> — ${f(s.mtdSales)} → ${f(s.mtdBuying)} · ${pctStr(s.pct)} ${flag(s.overBudget)} (${tail})\n`;
    }
  }

  msg += `\n💡 Check this before your weekend buying trip.`;
  return msg;
}
